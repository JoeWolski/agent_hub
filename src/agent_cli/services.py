        ...
        *,
        ...
        try:
            ...
            if runtime_bridge is not None:
                runtime_run_args = list(run_args)
                if bool(getattr(runtime_bridge, "mount_runtime_config", True)):
                ...
                for runtime_mount in getattr(runtime_bridge, "mounts", []):
                    runtime_run_args.extend(["--volume", str(runtime_mount)])
                for runtime_env in getattr(runtime_bridge, "env_vars", []):
...
class LaunchPipelineExecutor:
    ...
    def _compile_agent_command(self) -> list[str]:
        ...
        if self.data.selected_agent_provider == self.deps.agent_provider_codex:
            trusted_paths: list[str] = []
            workspace_root = str(PurePosixPath(self.data.container_project_path).parent)
            for candidate in (workspace_root, self.data.container_project_path):
                value = str(candidate or "").strip()
                if not value or value in trusted_paths:
                    continue
                trusted_paths.append(value)
            for trusted_path in trusted_paths:
                codex_project_trust_key = f"projects.{json.dumps(trusted_path)}.trust_level"
                if self.deps.has_codex_config_override(explicit_container_args, key=codex_project_trust_key):
                    continue
    ...
    def _run_rw_preflight(self) -> None:
        if not self.data.run_rw_mount_preflight or not self.rw_mount_specs:
            return
