...
from agent_core.errors import RuntimeCommandError, RuntimeStateError, TypedAgentError, typed_error_http_status, typed_error_payload
...
def _detect_primary_host_ip() -> str:
    try:
        ...
        if probe.returncode == 0 and candidate:
            normalized = _normalize_callback_forward_host(candidate)
            if normalized:
                return normalized
    ...
    try:
        ...
        return _normalize_callback_forward_host(sock.getsockname()[0])
    except OSError:
        return ""
    finally:
        sock.close()
...
def _auto_artifact_publish_base_url(*, hub_port: int) -> str:
    candidate_hosts: list[str] = []
    host_ip = _detect_primary_host_ip()
    if host_ip:
        candidate_hosts.append(host_ip)

    _linux_gateway, _linux_diagnostics = _discover_linux_default_gateway_host()
    docker_gateway, _docker_diagnostics = _discover_docker_bridge_gateway_host()
    normalized_docker_gateway = _normalize_callback_forward_host(docker_gateway)
    if normalized_docker_gateway and normalized_docker_gateway not in candidate_hosts:
        candidate_hosts.append(normalized_docker_gateway)

    default_host = _normalize_callback_forward_host(DEFAULT_ARTIFACT_PUBLISH_HOST)
    if default_host and default_host not in candidate_hosts:
        candidate_hosts.append(default_host)

    resolved_host = candidate_hosts[0] if candidate_hosts else DEFAULT_ARTIFACT_PUBLISH_HOST
    return f"http://{resolved_host}:{int(hub_port or DEFAULT_PORT)}"


def _can_bind_hub_port(host: str, port: int) -> bool:
...
def _resolve_hub_port(host: str, requested_port: int, *, auto_manage: bool) -> int:
    normalized_port = int(requested_port or DEFAULT_PORT)
    if _can_bind_hub_port(host, normalized_port):
        return normalized_port
    if not auto_manage:
        return normalized_port
    for candidate in range(normalized_port + 1, normalized_port + 51):
        if _can_bind_hub_port(host, candidate):
            return candidate
    return normalized_port
...
def _snapshot_setup_runtime_image_for_snapshot(snapshot_tag: str) -> str:
    return core_runtime_images.snapshot_setup_runtime_image_for_snapshot(
        snapshot_tag,
        error_factory=lambda message: ConfigError(message),
    )
...
def _build_runtime_image(*, base_image: str, target_image: str, agent_provider: str) -> None:
    core_runtime_images.build_runtime_image(
        repo_root=_repo_root(),
        dockerfile=AGENT_CLI_DOCKERFILE,
...
def _ensure_agent_cli_base_image_built() -> None:
    core_runtime_images.build_agent_cli_base_image(
        repo_root=_repo_root(),
        base_dockerfile=AGENT_CLI_BASE_DOCKERFILE,
...
def _ensure_runtime_image_built_if_missing(
    *,
    ...
    core_runtime_images.ensure_runtime_image_built_if_missing(
        ...
        agent_provider=agent_provider,
        repo_root=_repo_root(),
        runtime_dockerfile=AGENT_CLI_DOCKERFILE,
        base_dockerfile=AGENT_CLI_BASE_DOCKERFILE,
...
def _read_openai_api_key(path: Path) -> str | None:
    return core_runtime_images.read_openai_api_key(
        path,
        encoding="utf-8",
        errors="ignore",
...
def _upsert_codex_trusted_project_config(base_config_text: str, container_project_path: str) -> str:
    ...
    workspace_root = str(PurePosixPath(normalized_path).parent)
    for candidate in (workspace_root, normalized_path):
        value = str(candidate or "").strip()
        if not value:
            continue
        if not value.startswith("/"):
            value = "/" + value.lstrip("/")
        if value in trusted_paths:
            continue
        trusted_paths.append(value)
    cleaned = str(base_config_text or "")
    for trusted_path in trusted_paths:
        assignment_key = f"projects.{json.dumps(trusted_path)}.trust_level"
        pattern = re.compile(rf"(?m)^\s*{re.escape(assignment_key)}\s*=.*\n?")
        cleaned = re.sub(pattern, "", cleaned)
    rendered = cleaned.rstrip()
    for trusted_path in trusted_paths:
        assignment_key = f"projects.{json.dumps(trusted_path)}.trust_level"
...
class HubState(HubStateRuntimeMixin, HubStateOpsMixin):
    ...
    def __init__(
        ...
        build_hub_domain_service_bundle(
            ...
            forward_openai_callback_via_container_loopback_fn=lambda **kwargs: _forward_openai_callback_via_container_loopback(
                **kwargs
            ),
...
def main(
    ...
    parameter_source = click.get_current_context().get_parameter_source("port")
    auto_manage_port = parameter_source == ParameterSource.DEFAULT
    resolved_port = _resolve_hub_port(host, int(port), auto_manage=auto_manage_port)
    if resolved_port != int(port):
        click.echo(
            json.dumps(
                {
                    "event": "agent_hub_port_autoselect",
    ...
    resolved_artifact_publish_base_url = str(artifact_publish_base_url or "").strip()
    if not resolved_artifact_publish_base_url:
        resolved_artifact_publish_base_url = _auto_artifact_publish_base_url(hub_port=resolved_port)
    LOGGER.info(
    ...
    try:
        state = HubState(
            ...
            artifact_publish_base_url=resolved_artifact_publish_base_url,
    ...
    uvicorn.run(app, host=host, port=resolved_port, reload=reload, log_level=_uvicorn_log_level(normalized_log_level))
