...
def _normalized_chat_args(args: list[str] | None) -> list[str]:
    return [str(arg) for arg in (args or []) if str(arg).strip()]


def _request_config_mismatch(
    *,
    existing_chat: dict[str, Any],
    requested_agent_type: str,
    requested_agent_args: list[str],
) -> bool:
    existing_agent_type = str(existing_chat.get("agent_type") or "").strip()
    existing_agent_args = _normalized_chat_args(existing_chat.get("agent_args"))
    return existing_agent_type != str(requested_agent_type or "").strip() or existing_agent_args != requested_agent_args


def _log_create_request_lifecycle(
    _hub_server: Any,
    *,
    event: str,
    project_id: str,
    ...
    _hub_server.LOGGER.info(
        "chat_create_request event=%s project_id=%s request_id=%s chat_id=%s chat_status=%s agent_type=%s config_mismatch=%s",
        event,
        project_id,
        request_id,
        str(chat.get("id") or ""),
        _hub_server._normalize_chat_status(chat.get("status")),
        str(agent_type or ""),
        str(bool(config_mismatch)).lower(),
    )
...
class ProjectService:
    ...
    def _reconcile_stale_ready_project_for_chat_start(
        self,
        *,
        state: dict[str, Any],
        project_id: str,
        ...
        state["projects"][project_id] = project
        self._state.save(state, reason="project_build_reconcile")
        self._state._schedule_project_build(project_id)

    def _assert_project_chat_launch_ready(
        self,
        *,
        state: dict[str, Any],
        project_id: str,
        ...
        snapshot_ready, _snapshot_tag = launch_profile_service.chat_snapshot_ready(project)
        if snapshot_ready:
            return

        self._reconcile_stale_ready_project_for_chat_start(state=state, project_id=project_id, project=project)
        raise HTTPException(status_code=409, detail="Project image is not ready yet. Wait for setup build to finish.")

    def _start_chat_for_create_request(
        self,
        *,
        project_id: str,
        chat_id: str,
        ...
        try:
            ...
            if existing_chat is None or str(existing_chat.get("id") or "") != str(chat_id):
                raise
            normalized_existing_status = _hub_server._normalize_chat_status(existing_chat.get("status"))
            if normalized_existing_status not in {_hub_server.CHAT_STATUS_RUNNING, _hub_server.CHAT_STATUS_STARTING}:
                raise
    ...
    def create_and_start_chat(
        ...
        *,
        ...
        with self._state._chat_create_lock:
            state = self._state.load()
            project = state["projects"].get(project_id)
            if project is None:
                raise HTTPException(status_code=404, detail="Project not found.")
            ...
            if normalized_request_id:
                ...
                if existing_chat is not None:
                    normalized_existing_status = _hub_server._normalize_chat_status(existing_chat.get("status"))
                    config_mismatch = _request_config_mismatch(
                        existing_chat=existing_chat,
                        requested_agent_type=resolved_agent_type,
                        requested_agent_args=normalized_agent_args,
                    )
                    if normalized_existing_status in {_hub_server.CHAT_STATUS_RUNNING, _hub_server.CHAT_STATUS_STARTING}:
                        _log_create_request_lifecycle(
                            _hub_server,
                            event="create_request_reused_chat",
                            project_id=project_id,
                            request_id=normalized_request_id,
                            chat=existing_chat,
                            agent_type=str(existing_chat.get("agent_type") or resolved_agent_type),
                            config_mismatch=config_mismatch,
                        )
                        return existing_chat
                    self._assert_project_chat_launch_ready(state=state, project_id=project_id, project=project)
                    _log_create_request_lifecycle(
                        _hub_server,
                        event="create_request_restarted_existing_chat",
                        project_id=project_id,
                        request_id=normalized_request_id,
                        chat=existing_chat,
                        agent_type=str(existing_chat.get("agent_type") or resolved_agent_type),
                    ...
                    chat_id_to_start = str(existing_chat["id"])
                else:
                    self._assert_project_chat_launch_ready(state=state, project_id=project_id, project=project)
            else:
                self._assert_project_chat_launch_ready(state=state, project_id=project_id, project=project)
            if not chat_id_to_start:
                ...
                _log_create_request_lifecycle(
                    _hub_server,
                    event="create_request_new_chat",
                    project_id=project_id,
                    request_id=normalized_request_id,
                    chat=chat,
                    agent_type=resolved_agent_type,
                )
                chat_id_to_start = str(chat["id"])
        return self._start_chat_for_create_request(
            project_id=project_id,
            chat_id=chat_id_to_start,
            request_id=normalized_request_id,
        )
