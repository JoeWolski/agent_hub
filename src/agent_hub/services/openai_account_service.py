...
class OpenAIAccountService:
    ...
    def forward_openai_account_callback(
        ...
        callback_result = self._auth_forward_openai_account_callback_fn(
            session=session,
            callback_port=callback_port,
            callback_path=callback_path,
            query=query,
            artifact_publish_base_url=self._artifact_publish_base_url_getter(),
            request_host=request_host,
            request_context=request_context,
            discover_bridge_hosts=self._discover_bridge_hosts,
            normalize_host=self._normalize_callback_forward_host,
            callback_query_summary=self._openai_callback_query_summary,
            redact_url_query_values=self._redact_url_query_values,
            host_port_netloc=self._host_port_netloc,
            classify_callback_error=self._classify_callback_error,
            logger=self._logger,
        )
        ...
        return {
            ...
            "response_summary": self._short_summary(
                self._ansi_escape_re.sub("", callback_result.response_body),
                max_words=28,
                max_chars=220,
            ),
        }
