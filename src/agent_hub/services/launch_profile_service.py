...
class LaunchProfileService:
    ...
    def chat_snapshot_ready(self, project: dict[str, Any]) -> tuple[bool, str]:
    ...
    def assert_chat_snapshot_ready(self, project: dict[str, Any]) -> str:
        snapshot_ready, snapshot_tag = self.chat_snapshot_ready(project)
        if not snapshot_ready:
            raise HTTPException(status_code=409, detail="Project image is not ready yet. Wait for setup build to finish.")
        return snapshot_tag

    def _chat_launch_workspace(self, *, chat: dict[str, Any], project: dict[str, Any]) -> Path:
        project_id = str(project.get("id") or "").strip()
        if project_id:
            project_workspace = self._state.project_workdir(project_id)
            if project_workspace.is_dir() and (project_workspace / ".git").is_dir():
                return project_workspace
        return self._state._ensure_chat_clone(chat, project)
    ...
    def prepare_chat_launch_context(
        ...
        *,
        ...
        workspace = self._chat_launch_workspace(chat=chat, project=project)
