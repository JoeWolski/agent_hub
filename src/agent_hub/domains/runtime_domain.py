...
class RuntimeDomain:
    def __init__(
        ...
        *,
        ...
        self._chat_launch_tracking_lock = Lock()
        self._chat_launches_in_progress: set[str] = set()
    ...
    def mark_chat_launch_pending(self, chat_id: str) -> None:
        with self._chat_launch_tracking_lock:
            self._chat_launches_in_progress.add(str(chat_id))

    def clear_chat_launch_pending(self, chat_id: str) -> None:
        with self._chat_launch_tracking_lock:
            self._chat_launches_in_progress.discard(str(chat_id))

    def is_chat_launch_pending(self, chat_id: str) -> bool:
        with self._chat_launch_tracking_lock:
            return str(chat_id) in self._chat_launches_in_progress
