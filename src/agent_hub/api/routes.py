...
def register_hub_routes(
    ...
    *,
    ...
    def reject_legacy_codex_args(payload: dict[str, Any]) -> None:
    ...
    def api_openai_account_callback(request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            if not bool(payload.get("account_connected")):
                payload = state.auth_service.openai_account_session_payload()
            if bool(payload.get("account_connected")):
                payload["callback"] = {"forwarded": False, "non_fatal": True, "error": str(exc)}
                return payload
            raise
    ...
    async def api_start_new_chat_for_project(project_id: str, request: Request) -> dict[str, Any]:
        ...
        reject_legacy_codex_args(payload)
