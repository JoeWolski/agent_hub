...
class HubStateRuntimeMixin:
    ...
    def _discover_agent_capabilities_for_type(self, agent_type: str, previous: dict[str, Any]) -> dict[str, Any]:
        ...
        # Recent Codex CLI help output no longer enumerates model choices.
        # If discovery did not find any non-default models, use lightweight
        # Codex-specific fallbacks to populate model and reasoning dropdowns.
        if resolved_type == AGENT_TYPE_CODEX and _option_count_excluding_default(discovered_models) < 1:
            ...
            if _option_count_excluding_default(discovered_reasoning_modes) < 1:
                fallback_rc, fallback_output = _run_agent_capability_probe(
                    list(AGENT_CAPABILITY_CODEX_REASONING_FALLBACK_COMMAND),
                    AGENT_CAPABILITY_DISCOVERY_TIMEOUT_SECONDS,
                    docker_run_args=probe_run_args,
                )
                if fallback_rc != 127:
                    parsed_reasoning = _extract_reasoning_candidates_from_output(fallback_output, resolved_type)
                    if parsed_reasoning:
                        discovered_reasoning_modes = _normalize_reasoning_mode_options_for_agent(
                            resolved_type,
                            parsed_reasoning,
    ...
    def _chat_for_create_request(
        ...
        for chat in state.get("chats", {}).values():
            ...
            if _compact_whitespace(str(chat.get("create_request_id") or "")).strip() != normalized_request_id:
                continue
            matches.append(dict(chat))
        if not matches:
            return None

        if len(matches) > 1:
            LOGGER.warning(
                "chat_create_request_duplicate_match project_id=%s request_id=%s chat_ids=%s",
                normalized_project_id,
                normalized_request_id,
                ",".join(str(chat.get("id") or "") for chat in matches),
            )

        def match_sort_key(chat: dict[str, Any]) -> tuple[int, str, str]:
            normalized_status = _normalize_chat_status(chat.get("status"))
            status_rank = 2
            if normalized_status == CHAT_STATUS_RUNNING:
                status_rank = 0
            elif normalized_status == CHAT_STATUS_STARTING:
                status_rank = 1
            created_at = str(chat.get("created_at") or "")
            chat_id = str(chat.get("id") or "")
            return (status_rank, created_at or chat_id, chat_id)

        return min(matches, key=match_sort_key)
    ...
    def _delete_path_entry(self, path: Path) -> None:
        ...
        for child in root_dir.iterdir():
            ...
            try:
                ...
                if not ignore_delete_errors:
                    raise
                LOGGER.warning(
                    "Startup reconcile skipped orphan %s path=%s detail=%s",
                    orphan_kind,
                    child,
    ...
    def _remove_orphan_log_entries(
        self,
        state: dict[str, Any],
        *,
        ignore_delete_errors: bool = False,
        ...
        for entry in self.log_dir.iterdir():
            ...
            try:
                ...
                if not ignore_delete_errors:
                    raise
                LOGGER.warning(
                    "Startup reconcile skipped orphan log entry path=%s detail=%s",
                    entry,
                    str(exc.detail or exc),
                )
                continue
    ...
    def _ensure_chat_clone(self, chat: dict[str, Any], project: dict[str, Any]) -> Path:
        ...
        if workspace.exists():
            ...
            if git_dir.is_dir():
                return workspace
            self._delete_path_entry(workspace)
            ...
            if workspace.exists():
                try:
                    ...
                    raise RuntimeStateError(
                        f"Chat clone failed: {clone_exc}. Cleanup failed for {workspace}: {cleanup_exc}"
                    ) from clone_exc
            raise
        return workspace
...
<body>
  <header>
    <h1>Agent Hub</h1>
    <div class="subhead">Project snapshots build once; new chats launch from ready containers</div>
  </header>
  ...
  <script>
    ...
    async function deleteProject(projectId) {
      if (!confirm('Delete this project and all chats? This also removes shared project snapshots and chat workspaces.')) return;
