...
export function projectRowFromPendingAutoConfig(project) {
  return {
    ...
    build_status:
      project.auto_config_status === "failed" || project.auto_config_status === "cancelled"
        ? "failed"
        : "building",
...
export function markPendingAutoConfigProjectCancelled(projects, requestId, message) {
  const normalizedRequestId = String(requestId || "");
  const normalizedMessage = String(message || "");
  if (!normalizedRequestId) {
    return Array.isArray(projects) ? [...projects] : [];
